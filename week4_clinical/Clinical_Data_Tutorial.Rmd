...
Call the sum function on age_mask. Remember that TRUEs are treated as 1, and FALSEs are treated as 0. Knowing this, what will your sum represent?
    1. Sum = number of patients that are "young"
...
```{r}
nrow(clinic) == nrow(old_clinic) + nrow(young_clinic)
```
...
Let's create a column in clinical called survival_time. For some patients, we can find survival time using their days_to_death info. But for others, namely those who never had a registered death event (ie days_to_death = NA), our best estimate becomes days_to_last_follow_up, since that is the longest definitive time we can say they survived. Using the days_to_death and days_to_last_followup columns, create a survival_time column in the clinical data frame.
...
Those patients that are "young" (<50 yo) have better survival outcomes across essentially all time periods.
