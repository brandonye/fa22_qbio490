...
write.csv(clinical, "/Users/brandonye/Desktop/fa22/QBIO 490/fa22_qbio_490_brandon/analysis_data/brca_clinical_data.csv", row.names = FALSE)
```
...
```{r Exercise 3.4}
# write code here
head(maf_object@data$Tumor_Sample_Barcode[1:110])
head(maf_object@clinical.data$Tumor_Sample_Barcode[1:110])
```
...
tsb_hgvs_mask <- ifelse(colnames(maf_object@data) == "Tumor_Sample_Barcode" | colnames(maf_object@data) == "HGVSp_Short", T, F)

maf_object@data[tp53_mask, c(17,38)]
maf_object@data[tp53_mask, ..tsb_hgvs_mask]
```
...
```{r Exercise 3.7}
# write code here
head(maf_object@data$Mutation_Status)
maf_object@data$HGVSp_Short[maf_object@data$Hugo_Symbol == "TP53"]
```
