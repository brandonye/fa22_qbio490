...
rna_clinical <- read.csv("/Users/brandonye/Desktop/fa22/QBIO 490/fa22_qbio_490_brandon/analysis_data/brca_rna_clinical_data.csv", header = TRUE, row.names = 1)
rna_genes <- read.csv("/Users/brandonye/Desktop/fa22/QBIO 490/fa22_qbio_490_brandon/analysis_data/brca_rna_gene_data.csv", header = TRUE, row.names = 1)
rna_counts <- read.csv("/Users/brandonye/Desktop/fa22/QBIO 490/fa22_qbio_490_brandon/analysis_data/brca_rna_count_data.csv", header = TRUE, row.names = 1)
```
...
colnames(results) <- c(c("gene_name", "gene_id", "log2foldchange", "pvalue", "padj", "-log10padj")) ## FIX
```
...
```{r}
# write code here
row.names(up_reg_results) <- c(gsub(" ", "", paste("Gene ", 1:23221)))
row.names(down_reg_results) <- c(gsub(" ", "", paste("Gene ", 1:15084)))
```
...
Gene ensembl id: ENSG00000135222.6
Log2FoldChange: 2.88062474
p-adj value: 5.116414e-08
More highly expressed in young or old patients: Young
Google the gene. What is its full name, and what appears to be its function? Genecards generally has a pretty good summary: Leber Congenital Amaurosis 5 Protein encodes a protein that is thought to be involved in centrosomal or ciliary functions. Mutations in this gene cause Leber congenital amaurosis type V.

Gene ensembl id: ENSG00000161634.12
Log2FoldChange: -2.887474
p-adj value: 7.343876e-16
More highly expressed in young or old patients: Old
Google the gene. What is its full name, and what appears to be its function? Genecards generally has a pretty good summary: C-X-C Motif Chemokine Ligand 16 enables chemokine activity and is involved in several processes, including positive regulation of cell growth, response to interferon-gamma, and response to tumor necrosis factor.

Write the results, up_reg_results, and down_reg_results df's to analysis_data using write.csv().

```{r}
# write code here
write.csv(results, "/Users/brandonye/Desktop/fa22/QBIO 490/fa22_qbio_490_brandon/analysis_data/brca_deseq2_results.csv", row.names = TRUE)
write.csv(up_reg_results, "/Users/brandonye/Desktop/fa22/QBIO 490/fa22_qbio_490_brandon/analysis_data/brca_deseq2_up_reg_results.csv", row.names = TRUE)
write.csv(down_reg_results, "/Users/brandonye/Desktop/fa22/QBIO 490/fa22_qbio_490_brandon/analysis_data/brca_deseq2_down_reg_results.csv", row.names = TRUE)
```
...
# create plot here
# create plot here
jpeg("/Users/brandonye/Desktop/fa22/QBIO 490/fa22_qbio_490_brandon/week8_deseq2/volcano_plot.jpg")
par(mar=c(1,1,1,1))
EnhancedVolcano(results,
    lab = rownames(results),
    x = 'log2foldchange',
